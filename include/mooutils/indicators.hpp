...
auto point_hv(P const& p, R const& r) {
  using result_t = std::common_type_t<typename P::value_type, typename R::value_type>;
  return std::transform_reduce(p.begin(), p.end(),   // noformat
                               r.begin(),            // noformat
                               result_t{1},          // noformat
                               std::multiplies<>{},  // noformat
                               std::minus<>{});
}
...
//   using result_t = std::common_type_t<typename S::value_type::value_type, typename
//   R::value_type>; result_t res = 0; result_t aux = r[1]; for (auto const& v : s) {
//     res += (v[1] - aux) * (v[0] - r[0]);
//     aux = v[1];
//   }
//   return res;
...
//   using result_t = std::common_type_t<typename S::value_type::value_type, typename
//   R::value_type>; using array2_t = std::array<result_t, 2>;
...
//  public:
//   using value_type = T;
//   using objective_vector_type = std::conditional_t<N == std::dynamic_extent,  // noformat
//                                                    std::vector<value_type>,   // noformat
//                                                    std::array<value_type, N>>;
//   using set_type = mooutils::sets::sorted_multivector<objective_vector_type>;

//   constexpr explicit hypervolume(objective_vector_type&& r)
//       : m_hv(0)
//       , m_set()
//       , m_ref(std::move(r)) {}
...
//     return m_inclhv(v, m_ref) - m_wfg<N>(m_limitset(m_set, get_objective_vector(v)), m_ref);
//   }
...
//   auto m_limitset(S const& s, V const& v) const {
//     S res;
//     for (auto const& p : s) {
//       auto aux = p;
//       std::transform(v.begin(), v.end(), aux.begin(), aux.begin(), std::min);
...
//   auto m_inclhv(V const& v, R const& r) const {
//     return std::transform_reduce(v.begin(), v.end(),   // noformat
//                                  r.begin(),            // noformat
//                                  value_type{1},        // noformat
//                                  std::multiplies<>{},  // noformat
//                                  std::minus<>{});
//   }

//   // Assumes points are lexicographically sorted in worsening order.
//   template <typename S, typename R>
//   auto m_sorted_set_hv2d(S const& s, R const& r) const {}
...
//   // Assumes points in set are lexicographically sorted
//   template <std::size_t I, typename S, typename R>
//   auto m_wfg(S const& s, R const& r, value_type c = 1) const -> value_type {
//     value_type v = 0;
...
//         v += newc * m_point_hv(newp, newr) - m_set_hv<I - 1>(m_limit_set(newl, newp), newr,
//         newc);
